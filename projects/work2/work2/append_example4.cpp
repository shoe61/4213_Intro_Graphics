...
Cube::Cube()
{
   ...
   //this series of commands divides the six faces into triangles
   quad(1, 0, 3, 2);
   quad(2, 3, 7, 6);
   quad(3, 0, 4, 7);
   quad(6, 5, 1, 2);
...
//    to the vertices
//made Index a private class variable
//int Index = 0;
/*void
...
init()
{
   //THIS FUNCTION HAS BEEN INCORPORATED IN CONSTRUCTOR
   //colorcube();
    ...
    // Create and initialize a buffer object--COPIED TO CUBE::LOAD
    /*GLuint buffer;
    glGenBuffers( 1, &buffer );
    glBindBuffer( GL_ARRAY_BUFFER, buffer );
    glBufferData( GL_ARRAY_BUFFER, sizeof(points) + sizeof(colors),
